...
public class PlanningIlluminationApi {
  public record Evaluate(long assumptionsVersion) {
    public Evaluate { if (assumptionsVersion <= 0) throw new IllegalArgumentException("Exact assumptions version required"); }
  }
  public record CandidateResult(String candidateId, JsonNode ownerResult) {}
  public record Assessment(String runId, String runSha256, long assumptionsVersion,
      List<CandidateResult> candidates) {
    public Assessment { candidates = List.copyOf(candidates); }
  }
  ...
  public PlanningIlluminationApi(StateStore store, ServiceHttp http, Json json) {
    this.store = store; this.http = http; this.json = json;
  }

  @ExceptionHandler(org.springframework.web.client.HttpClientErrorException.NotFound.class)
  @ResponseStatus(org.springframework.http.HttpStatus.NOT_FOUND)
  public ApiErrors.Error missingOwner() { return new ApiErrors.Error("NOT_FOUND", "Required illumination artifact not found"); }

  @PostMapping("/api/planning/runs/{id}/illumination")
  @PreAuthorize("hasAnyRole('ADMIN','OPERATOR','SERVICE')")
  public JsonNode evaluate(@PathVariable String id, @RequestBody Evaluate request,
      @RequestHeader("Idempotency-Key") String key, Authentication actor) {
    String scope = "planning-illumination:" + id + ":" + actor.getName();
    ...
    return store.idempotent(scope, key, request, () -> {
      ...
      store.event("PlanningIlluminationEvaluated", run.requestId(), run.requestRevision(), UUID.randomUUID(), null, saved);
      return saved;
    });
  }
  ...
  Assessment compute(PlanningRuns.Published run, long version) {
    ...
    String digest = run.geometry().value().path("prediction").path("body").path("referenceDigest").asText();
    ...
    var source = http.get("mission-definition", "/internal/solar-interval-assumptions/"
        + PlanningInputs.segment(run.spacecraftId()) + "/versions/" + version, JsonNode.class);
    if (!run.spacecraftId().equals(source.path("id").asText()) || !source.path("version").isIntegralNumber()
        || !source.path("version").canConvertToLong() || source.path("version").asLong() != version)
    ...
    var target = new Aoi(area.id(), area.west(), area.east(), area.south(), area.north(), assumptions.extent().altitudeMeters());
    var results = new ArrayList<CandidateResult>();
    for (var candidate : run.run().candidates()) {
      var query = new TargetIlluminationQuery(target, candidate.activity().window(), model.minimumSunElevationDegrees());
      assumptions.requireCoverage(assumptions.solarModel(), digest, target, query.horizon());
      var body = Map.of("spacecraftId", run.spacecraftId(), "assumptionsVersion", version, "query", query);
      var result = http.post("flight-dynamics", "/internal/solar-intervals", body,
          "planning-solar-" + json.fingerprint(body), JsonNode.class);
      var value = result.path("body");
      if (result.path("id").asText().isBlank() || result.path("version").asLong() < 1
          ...
          || !Set.of("SUPPORTED_BY_DECLARED_ASSUMPTIONS", "NOT_ESTABLISHED").contains(value.path("outcome").asText()))
  ...
  @GetMapping({"/api/planning/runs/{id}/illumination/{version}", "/internal/planning/runs/{id}/illumination/{version}"})
  @PreAuthorize("hasAnyRole('ADMIN','OPERATOR','SERVICE')")
  public StateStore.State<Assessment> read(@PathVariable String id, @PathVariable long version) {
    return store.require("planning-illumination", json.fingerprint(List.of(id, version)), Assessment.class);
  }
}
